...
def create_train_sample(doc, neg_entity_count: int, neg_rel_count: int, max_span_size: int, rel_type_count: int, docs_len, k=10):
    ...
    # (int) doc.doc_id
    # remember add k & docs_len & crd_samples_idx ...
    neg_docs = [i for i in range(0, docs_len) if i != doc.doc_id]
    replace = True if k > len(neg_docs) else False
    neg_sample_idx = np.random.choice(neg_docs, k, replace=replace)
    crd_samples_idx = np.hstack((np.asarray(doc.doc_id),neg_sample_idx))
    crd_samples_idx = torch.tensor(crd_samples_idx, dtype=torch.long)
    return dict(encodings=encodings, context_masks=context_masks, entity_masks=entity_masks,
                ...
                entity_sample_masks=entity_sample_masks, rel_sample_masks=rel_sample_masks, crd_samples_idx=crd_samples_idx)
