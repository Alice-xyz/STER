...
class SpERT(BertPreTrainedModel):
    ...
    def _forward_train(self, encodings: torch.tensor, context_masks: torch.tensor, entity_masks: torch.tensor,
        ...
        # add later// entity_features, rel_features
        entity_clf, entity_spans_pool, entity_features = self._classify_entities(encodings, h, entity_masks, size_embeddings)

        # classify relations
        h_large = h.unsqueeze(1).repeat(1, max(min(relations.shape[1], self._max_pairs), 1), 1, 1)
        rel_clf = torch.zeros([batch_size, relations.shape[1], self._relation_types]).to(
            self.rel_classifier.weight.device)  # new build, need add to device
        rel_features = []  #
        ...
        for i in range(0, relations.shape[1], self._max_pairs):
            ...
            chunk_rel_logits, chunk_rel_repr = self._classify_relations(entity_spans_pool, size_embeddings,
                                                        relations, rel_masks, h_large, i)
            rel_clf[:, i:i + self._max_pairs, :] = chunk_rel_logits
            rel_features.append(chunk_rel_repr)
        rel_features = torch.cat(rel_features, dim=1)
        return entity_clf, rel_clf, entity_features, rel_features
    ...
    def _forward_eval(self, encodings: torch.tensor, context_masks: torch.tensor, entity_masks: torch.tensor,
        ...
        return entity_clf, rel_clf, relations, entity_features, rel_features
    ...
    def _classify_entities(self, encodings, h, entity_masks, size_embeddings):
        ...
        entity_repr = self.dropout(entity_repr)  # the entity_representation in penultimate layer

        # classify entity candidates
        entity_clf = self.entity_classifier(entity_repr)

        return entity_clf, entity_spans_pool, entity_repr
    ...
    def _classify_relations(self, entity_spans, size_embeddings, relations, rel_masks, h, chunk_start):
        ...
        entity_pairs = entity_pairs.view(batch_size, entity_pairs.shape[1], -1)  # potential entity representation in penultimate layer
        ...
        rel_repr = self.dropout(rel_repr)  # potential relation representation in penultimate layer
